...
socket.on("state", s => {
  ...
  s.board.forEach((t,i)=>{
    ...
    d.innerHTML += `<b>${t.name}</b>${t.price?`<br>₹${t.price}`:""}`;
    ...
    if(t.owner){
      ...
      d.innerHTML += `<div class="house">${"🏠".repeat(t.houses||0)}</div>`;
    }

    if(s.awaitingBuy && s.awaitingBuy.tileIndex===i && s.awaitingBuy.playerId===socket.id){
  ...
  s.players.forEach(p=>{
    players.innerHTML += `${p.name}: ₹${p.money} | 🎲 ${p.lastDice}<br>`;
  });
});
</script>
